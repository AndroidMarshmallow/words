...
public class AbsenceInfoAdapterForManager extends
        RecyclerView.Adapter<AbsenceInfoAdapterForManager.AbsenceViewHolder> {
    ...
    public AbsenceInfoAdapterForManager(List<Absence> list, OnItemClickListener listener, OnLinerLayoutClickListener onLinerLayoutClickListener, Context context) {
    ...
    @Override
    public AbsenceInfoAdapterForManager.AbsenceViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.absence_info_item_for_manager, parent, false);
        AbsenceViewHolder pvh = new AbsenceViewHolder(v);
        return pvh;
    }

    @Override
    public void onBindViewHolder(AbsenceInfoAdapterForManager.AbsenceViewHolder holder, int position) {
    ...
    public void logbooksWriter(AbsenceViewHolder holder,Absence absence) throws ParseException {
        if(absence.getLogbooks().size()<2) {
            ...
            if (absence.getLogbooks().get(0).getNote().isEmpty()) {
